 ...
 * be used with the for/await loop (which will not terminate until the
 * close() method is called.)
 */

class AsyncQueue {
  constructor() {
    // Values that have been queued but no dequeued yet are stored here
    this.values = [];
    // When Promises are dequeued before their corresponding values are
    // queued, the resolve methods for those Promise are stored here.
    this.resolvers = [];
  ...
  enqueue(value) {
    ...
    if (this.resolvers.length > 0) {
      // If this value has alread been promised, resolve that Promise
  ...
  // dequeue() Promise resolves to a value or the EOS sentinel if we'e
