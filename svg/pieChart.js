 ...
 *
 * This function expets an object argument with the following
 * properties:
 *
 *  width, height: the size of the SVG graphic, in pixels
 *  cs, cy, r: the center and radius of the pie
 ...
 *
 * The function returns an <svg> element. The caller must insert it
 * into the document in order to make it visible.
 */
function pieChart(options) {
  ...
  // values unset here, they can be set with CSS instedad.
  chart.setAttribute("font-family", "sans-serif");
  chart.setAttribute("font-size", "18");

  // Get labels and values as arrays and add up the values so we know
  // how big the pie is.
  ...
  // angles[i] and ends at angles[i+1]. The angles are measured in
  // radians.
  let angles = [0];
  ...
  values.forEach((value, i) => {
    ...
    // This string describes hwo to draw a slice of the pie chart:
    let path =
    ...
    let color = `hsl(${(i * 40) % 360},${90 - 3 * 1}%,${50 + 2 * i}%)`;
